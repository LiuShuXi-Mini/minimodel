...
--编写方法：可以使用new函数来加载类，然后直接复制进迷你世界，或者直接调用class类中的函数然后用源码自带转换器转换一下，再复制到迷你世界。
--变量命名限定:
...
--增加游戏规则
--1.3
--修复语法错误（真的没有看到VScode的找错。。现在看到了，抱歉）
--1.2
--让导出更易于使用
--1.1
--两个语法错误
...
function class.MiniTimer:new()
    local ret = {}
    setmetatable(ret,class.MiniTimer)
    return ret
end
...
--你是来编代码的吗？
--------------------------------------------导入时上面不许复制-------------------------------------------------
--这是一个例程，就是起床战争，开发者可以以这个为起点开发。
--起床战争模板
--事件
main = function()
    class.Chat:sendSystemMsg("欢迎！Copyrright(C)68066216,83552953等3个",0)--版权声明，写上自己的，最后个引用了这个就行
    class.Chat:sendSystemMsg("玩法:挖掉对方的旗子就可以胜利，如果己方的旗子被破坏，则对方胜利。",0)
    class.Chat:sendSystemMsg("初始化变量中...",0)
    di()
    class.Chat:sendSystemMsg("注册计时器中")
    MAIN_TIMER_HWN = dtimer()
    class.Chat:sendSystemMsg("加载完成，尽情玩耍吧！",0)
end
enterg = function(eventobjid,toobjid)
    local re,na = class.Player.getNickname(eventobjid)
    if (re == ErrorCode.OK) then
        class.Chat.sendSystemMsg("欢迎"..na.."!".."注意！撸掉自己家的旗子会失效！")
    end
end
timerc = function(timerid,timername)
    if (class.MiniTimer.getTimerTime(MAIN_TIMER_HWN) % 5 == 0) then--每5s执行一次，更改“5”为其他数字可以调整频率
        class.World.spawnItem(-228,8,-2,11209,1)--11209：铁 请更改前3个参数为铁生成的地址
        class.World.spawnItem(18,8,5,11209,1)--11209：铁 请更改前3个参数为铁生成的地址
        class.World.spawnItem(-105,7,1,11203,1)--11203：钻石 请更改前3个参数为钻石生成的地址
        class.World.spawnItem(-105,7,1,200,1)--200：木头 请更改前3个参数为木头生成的地址
        class.World.spawnItem(-105,7,1,706,1)--706：电能线 请更改前3个参数为电能线生成的地址
        --todo:增加或减少地址
    end
end
bldes = function(eventobjid, blockid, x, y, z)
    ...
    --不要再这里写代码
    local re = 10001
    class.Chat.sendSystemMsg(blockid,0)
    if (blockid == 2002) then--把2002改成红方要保护的方块id
        repeat
            class.Chat:sendSystemMsg("红队胜利！",0)
            re = class.Team.setTeamScore(1,100)
        until re == ErrorCode.OK
    end if (blockid == 2001) then--把2001改成蓝方要保护的方块id
        repeat
            class.Chat:sendSystemMsg("蓝队胜利！",0)
            re = class.Team.setTeamScore(2,100)
        until re == ErrorCode.OK
    end
end
--注册函数事件和初始化
function revent()
    class.Chat.sendSystemMsg("事件：Game.AnyPlayer.EnterGame，返回函数：enterg")
    ScriptSupportEvent:registerEvent([=[Game.AnyPlayer.EnterGame]=],enterg)
    class.Chat.sendSystemMsg("事件：minitimer.change，返回函数：timerc")
    ScriptSupportEvent:registerEvent([=[minitimer.change]=],timerc)
    class.Chat:sendSystemMsg("事件：Player.ClickBlock，返回函数：bldes")
    ScriptSupportEvent:registerEvent([=[Player.ClickBlock]=],bldes)
    class.Chat.sendSystemMsg("注册初始事件中Game.Start，加载到main")
    ScriptSupportEvent:registerEvent([=[Game.Start]=],main)
    class.Chat.sendSystemMsg("事件注册完成")

end
function di()
    --TODO:有变量需要开头初始化就在这里初始化
    class.Chat.sendSystemMsg("变量初始化完成")
end
function dtimer()
    local re = 0
    local hwn = 0
    class.Chat:sendSystemMsg("名称：主计时器，类型，公有")
    repeat
        re,hwn = class.MiniTimer:createTimer("主计时器")
    until re == ErrorCode.OK
    class.Chat:sendSystemMsg("名称：主计时器，类型：正向启动")
    repeat
        re = class.MiniTimer:startForwardTimer(hwn)
    until re ==ErrorCode.OK
    class.Chat.sendSystemMsg("计时器初始化完成")
    return hwn
end
class.Chat.sendSystemMsg("加载游戏规则")
class.Chat.sendSystemMsg("规则：EndTime，值：20")
GameRule.EndTime = 20  		 --游戏时长
class.Chat.sendSystemMsg("规则：TeamNum，值：2")
GameRule.TeamNum = 2         --队伍数量
class.Chat.sendSystemMsg("规则：MaxPlayers，值：6")
GameRule.MaxPlayers = 6      --最大玩家量
class.Chat.sendSystemMsg("规则：StartMode，值：0")
GameRule.StartMode = 0       --开启模式 0:主开
class.Chat.sendSystemMsg("规则：StartPlayers，值：1")
GameRule.StartPlayers = 1    --最低玩家量 2人
class.Chat.sendSystemMsg("规则：PlayerDieDrops，值：0")
GameRule.PlayerDieDrops = 1  --死亡掉落 1:true
class.Chat.sendSystemMsg("规则：DisplayScore，值：1")
GameRule.DisplayScore = 1    --显示比分 1:true
class.Chat.sendSystemMsg("规则：AllowMidwayJoin，值：1")
GameRule.AllowMidwayJoin = 1 --中途加入 1:允许
class.Chat.sendSystemMsg("规则：ScoreKillPlayer，值：0")
GameRule.ScoreKillPlayer = 0 --击杀玩家 得0分
class.Chat.sendSystemMsg("规则：WinLoseEndTime，值：1")
GameRule.WinLoseEndTime = 1  --游戏超时结束则全胜
class.Chat.sendSystemMsg("规则：ScoreKillMob，值：0")
GameRule.ScoreKillMob = 0
class.Chat.sendSystemMsg("规则：CurTime，值：0")
GameRule.CurTime = 0;	--获取/设置当前时间
class.Chat.sendSystemMsg("规则：TimeLocked，值：1")
GameRule.TimeLocked= 1;	--时间锁/锁定时间
class.Chat.sendSystemMsg("规则：MobGen，值：1")
GameRule.MobGen= 1;	--是否刷怪: -1:按创建的选项刷, 0:不刷, 1:刷
class.Chat.sendSystemMsg("规则：EndScore，值：100")
GameRule.EndScore= 100;	--胜利分数设定
--TODO:增加或减少规则
class.Chat:sendSystemMsg("注册事件中...",0)
revent()
--小游戏1
...
etg = function(eventobjid,toobjid)
    class.Chat:sendSystemMsg("欢迎"..class.Player.getNickname(toobjid).."!"..Def.welgg,0)
end
