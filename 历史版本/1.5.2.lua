...
etg = function(eventobjid,toobjid)
    class.Chat:sendSystemMsg("欢迎"..class.Player.getNickname(eventobjid).."!"..Def.welgg,0)
end
pht = function(eventobjid,toobjid)
    if (class.Player:getAttr(toobjid) < 50) then
        class.Player:notifyGameInfo2Self(toobjid,"#R警告！生命值小于50%")
    end
end
ded = function(eventobjid,toobjid)
    --死亡一次减去一分
    class.Team:setTeamScore(1,class.Team:getTeamScore(class.Player:getTeam(toobjid) - 1))
end
tmc = function(timerid, timername)
    if class.MiniTimer:getTimerTime(Def.maintr) % Def.spl == 0 then
        re = ErrorCode.FAILED
        repeat
            re = class.World.spawnCreature(Def.sgx,Def.sgy,Def.sgz,Def.gw,Def.sgs)
        until re == ErrorCode.OK
        class.Chat:sendSystemMsg("刷怪了，做好准备",0)
        re = ErrorCode.FAILED
    end
    if (class.MiniTimer:getTimerTime(Def.maintr) % Def.bpl == 0) then
        if (nowBsT % 10 == 0) then
            repeat
                re = class.World.spawnCreature(Def.bosX,Def.bosY,Def.bosZ,Def.bos3)
            until re == ErrorCode.OK
            nowBst = nowBsT + 1
            class.Chat:sendSystemMsg("出现3级boos！",0)
            return
        end
        if (nowBsT % 5 == 0) then
            repeat
                re = class.World.spawnCreature(Def.bosX,Def.bosY,Def.bosZ,Def.bos2)
            until re == ErrorCode.OK
            nowBst = nowBsT + 1
            class.Chat:sendSystemMsg("出现2级boos！",0)
            return
        end
        repeat
            re = class.World.spawnCreature(Def.bosX,Def.bosY,Def.bosZ,Def.bos1)
        until re == ErrorCode.OK
        nowBsT = nowBsT +1
        class.Chat:sendSystemMsg("出现1级boos！",0)
    end
end
function revent ()
...
function rtimer()
    re = ErrorCode.FAILED
    repeat
        re,Def.maintr = class.MiniTimer:createTimer("main")
    until re == ErrorCode.OK
    re = ErrorCode.FAILED
    repeat
        re = class.MiniTimer:startForwardTimer(Def.maintr)
    until re == ErrorCode.OK
end
revent()
